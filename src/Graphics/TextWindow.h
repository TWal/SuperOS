...
namespace video{
    ...
    class TextWindow : public Window, public Stream{
        ...
        struct Line{
            ...
            explicit Line(){}
            /*Line(const Line& l) : data(l.data){
                fprintf(stderr,"Line copy\n");
            }
            Line(Line&& l) : data(std::move(l.data)){
                fprintf(stderr,"Line move\n");
                }*/
    ...
    public:
        ...
        TextWindow(Vec2u offset, Vec2u size, Font* font)
            : Window(offset,size), _font(font),
              ...
              _state(State::NORMAL), _bright(false)
            {
        ...
        u64 getMask() const {return Stream::READABLE | Stream::WRITABLE |
                Stream::APPENDABLE;}
        /// @todo implement that (keyboard manipualtion)
        size_t read(void* buf, size_t count) const{(void)buf; (void)count; return -1;};
        /// @todo implement that (keyboard manipualtion)
